 ...
 * Scaling is such that the position of the origin on the screen does not change.
 * Has methods for clearing background, painting axis, grid, scales and elapse time.
...
implements MouseListener, ComponentListener, MouseWheelListener, MouseMotionListener, KeyListener {
    ...
    public AbstractCanvas(double sx,double sy,int offx,int offy) {
        ...
        MenuItem resetBut = new MenuItem("Reset");
        popup.add(resetBut);
        ...
        CheckboxMenuItem scales = new CheckboxMenuItem("Show scale",showScale);
        popup.add(scales);
        ...
        CheckboxMenuItem grid = new CheckboxMenuItem("Show grid",showGrid);
        popup.add(grid);
        ...
        CheckboxMenuItem times = new CheckboxMenuItem("Show times",showTime);
        popup.add(times);
    ...
    /** Called when scale, or offset changed 
     * Default action is to call repaint */
    protected void rescaled() {
        repaint();
    }

    /** Called when canvas sized changes 
     * Default action is to call repaint */
    ...
    protected int clipX(int xAbsolute) {
        int res = xAbsolute;
        if (res > dimensions.width) res = dimensions.width;
        if (res < -1)	    	res = -1;
        return res;
    }
    ...
    protected int clipY(int yAbsolute) {
        int res = yAbsolute;
        if (res > dimensions.height) res = dimensions.height;
        if (res < -1)	    	res = -1;
        return res;
    }
     ...
     * unless they want a different buffering strategy.
     * Instead they should implement paintCanvas.
     ...
     * 
     * @param g
     * @param t1 start time in nano-secs.
     * @param t2 end time in nano-secs.
     */
    ...
    protected void paintAxes(Graphics g) {
        ...
        g.drawLine(	0,			yZero,
                dimensions.width-1,	yZero);
        g.drawLine(	xZero,	0,
                xZero,	dimensions.height-1);
    }    	
    ...
    protected void paintScale(Graphics g) {
        ...
        int yZero = this.yAbsolute(0.0);
        int xZero = this.xAbsolute(0.0);
        if(yZero >= dimensions.height -6) yZero = dimensions.height-6;
        if(yZero < 0) yZero = 0;
        if(xZero >= dimensions.width) xZero = dimensions.width-1;
        if(xZero < 5) xZero = 5;
        ...
        for(double x=xleft;x<=xright;x+=xinc) {
            ...
            g.drawLine(xAbs,	yZero,
                    xAbs, yZero + 5);
        }

        int ypos = yZero+5+fm.getHeight();
        if(ypos < 5) ypos = 5;
        if(ypos > dimensions.height - 10) ypos = dimensions.height - 10;
    ...
    public void mouseDragged(MouseEvent e) {
        //System.out.println(e);
        Point relp = e.getPoint();
        if(lastpos==null) { lastpos = relp; return; }
        int xdiff = relp.x - lastpos.x;
        int ydiff = relp.y - lastpos.y;
        lastpos=relp;
        shiftCanvas(xdiff,ydiff);
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if(code == KeyEvent.VK_LEFT) shiftCanvas(-val,0);
        if(code == KeyEvent.VK_RIGHT) shiftCanvas(val,0);
        if(code == KeyEvent.VK_UP) shiftCanvas(0,-val);
        if(code == KeyEvent.VK_DOWN) shiftCanvas(0,val);
        if(code == KeyEvent.VK_HOME) resetCanvas();
        if(code == KeyEvent.VK_PAGE_UP) zoomCanvas(5);
        if(code == KeyEvent.VK_PAGE_DOWN) zoomCanvas(-5);
    }
